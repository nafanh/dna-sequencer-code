...
for k in range(len(length_dir_name)):
    #Takes first time point as the standard reference peak
    if k == 0:
        first_dp_split = length_dir_name[0].split('_')
        time = first_dp_split[2]
        standard = SeqIO.read(length_dir_name[0],'abi')
    ...
    name_split = length_dir_name[k].split('_')
    time_peak = name_split[2]

    #opens up the FSA file
    record = SeqIO.read(length_dir_name[k],'abi')
