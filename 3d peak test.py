...
for k in range(length_dir):
    ...
    ##
    ##    
        sc_std = ax.plot(x_values_time,y_values,z_values)
        continue
    ...
    #ax = fig.add_subplot(3,3,subplot_num,projection='3d')
    sc_non_std = ax.plot(x_values_time_non_std,y_values_non_std,z_values_non_std)
...
###fig,ax = plt.subplots(2,num_pts//2,sharex=True,sharey=True,projection='3d')
##num_rows = 2
##num_columns = num_pts//2
##fig = plt.figure()
##
##
...
##j = 1
##count = 1
###s_trace['DATA1'] = y values, array = x values
...
##        ax = fig.add_subplot(num_rows,num_columns,count,projection='3d')
##        first_dp_split = fsa_names[k].split('_')
##        time = first_dp_split[2]
##        standard = SeqIO.read(fsa_names[k],'abi')
##        s_abif_key = standard.annotations['abif_raw'].keys()
...
####                continue
##    #Displays the peaks
##    #Array variable is x values trace['DATA1'] are y values
